...
int __init snd_soc_util_init(void)
{
	...
	dev_dbg(&soc_dummy_dev->dev, "lin test 1 snd_soc_util_init dummy_name = %s\n",
	 dev_name(&soc_dummy_dev->dev));
	...
	dev_dbg(&soc_dummy_dev->dev, "lin test 2 snd_soc_util_init dummy_name = %s\n", 
	dev_name(&soc_dummy_dev->dev));
	...
	ret = snd_soc_register_dai(&soc_dummy_dev->dev, &voice_dai);
	if (ret) {
		platform_device_put(soc_dummy_dev);
